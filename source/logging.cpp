...
void Logging::createLogFile(void) {
	if((access("sdmc:/BetterDex/BetterDex.log", F_OK) == 0)) {
	} else {
		FILE* logFile = fopen(("sdmc:/BetterDex/BetterDex.log"), "w");
		fclose(logFile);
	}
}
...
void Logging::writeToLog(std::string debugText) {
	...
	logFile.open(("sdmc:/BetterDex/BetterDex.log"), std::ofstream::app);
